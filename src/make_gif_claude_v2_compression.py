...
def create_transparent_parallax_frames(
    ...
    """
    Create animation frames showing parallax effect with transparent background (stars only)
    ...
    parallax_mode : str, optional
        Mode for scaling parallax with distance ('logarithmic', 'inverse', 'power')
    contrast_factor : float, optional
        Factor to enhance contrast between near and far star movements
    power : float, optional
        Power factor for 'power' parallax mode
    ...
    --------
    list
        List of PIL.Image frames with transparent background
    """
    height, width = original_image.shape[:2]
    frames = []
    ...
    logger.info(f"Generating {num_frames} transparent animation frames")
    for frame_idx in tqdm(range(num_frames)):
        ...
        # Copy star pixels to transparent frame
        if len(shifted_stars.shape) == 3 and shifted_stars.shape[2] >= 3:
            transparent_frame[:, :, :3] = shifted_stars[:, :, :3]
        else:
            # For grayscale stars, copy to all RGB channels
            for c in range(3):
                transparent_frame[:, :, c] = shifted_stars

        # Set alpha channel based on star presence
        if len(shifted_stars.shape) == 3 and shifted_stars.shape[2] == 4:
            # Use existing alpha
            transparent_frame[:, :, 3] = shifted_stars[:, :, 3]
        else:
            ...
            if len(shifted_stars.shape) == 3:
                ...
                luminance = shifted_stars
            transparent_frame[:, :, 3] = np.where(luminance > 0, 255, 0)

        # Convert to PIL Image
        frame = Image.fromarray(transparent_frame, 'RGBA')
    ...
    logger.info(f"Generated {len(frames)} transparent animation frames")
    return frames
...
def parse_arguments():
    ...
    parser.add_argument('--duration', '-d', type=int, help='Frame duration in milliseconds', default=50)  # Changed to your preferred 50ms
    ...
    parser.add_argument('--optimization-level', choices=['low', 'medium', 'high'],
                      help='Level of GIF size optimization', default='medium')
    parser.add_argument('--downscale', type=float,
                      help='Factor to downscale images (e.g., 0.5 for half size)', default=1.0)
    parser.add_argument('--lossy', type=int, choices=range(0, 201),
...
def main():
    ...
    try:
        ...
        # Choose optimization strategy based on size reduction preference
        if args.optimization_level == 'high':
            # Maximum optimization using delta frames and gifsicle
            save_efficient_gif(frames, args.output, duration=args.duration)
        elif args.optimization_level == 'medium':
            # Basic optimization with PIL and gifsicle
